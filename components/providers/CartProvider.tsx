...
// Reducer - prevents direct state mutations
...
export function MembershipCartProvider({
  ...
  // Refresh cart data from server - MEMOIZED to prevent infinite loops
  const refreshCart = useCallback(async () => {
    try {
      ...
      if (cartResult.success) {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: cartResult.error || "Failed to load cart",
        });
      }
      ...
      if (summaryResult.success) {
        dispatch({
          type: "SET_SUMMARY",
          payload: summaryResult.summary || null,
        });
      }
  ...
  const addToCart = useCallback(
    async (productId: string, quantity: number = 1): Promise<boolean> => {
      try {
        ...
        if (result.success) {
          await refreshCart(); // Refresh to get latest data
          return true;
        } else {
          dispatch({
            type: "SET_ERROR",
            payload: result.error || "Failed to add item",
          });
          return false;
        }
      } catch (error) {
    ...
    async (productId: string, quantity: number): Promise<boolean> => {
      try {
        ...
        if (result.success) {
          ...
          dispatch({
            type: "SET_ERROR",
            payload: result.error || "Failed to update item",
          });
          return false;
        }
      } catch (error) {
  ...
  const clearCart = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (result.success) {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: result.error || "Failed to clear cart",
        });
        return false;
      }
    } catch (error) {
  ...
  // Initialize cart on mount - CONTROLLED to prevent infinite loops
  useEffect(() => {
    let mounted = true;
  ...
  }, [refreshCart]); // Only run once because refreshCart is memoized
...
export function useMembershipCart() {
  ...
  if (context === undefined) {
    throw new Error(
      "useMembershipCart must be used within a MembershipCartProvider"
    );
  }
  return context;
}

// Custom hook with computed values - PREVENTS INFINITE LOOPS
export function useMembershipCartState() {
  ...
  // MEMOIZED computed values - prevents unnecessary re-calculations
  const computedValues = React.useMemo(() => {
    ...
    const stats = {
      ...
      membershipSavings:
        summary?.membershipSavings || cart?.membershipDiscount || 0,
      promotionSavings:
        summary?.promotionSavings || cart?.promotionDiscount || 0,
      totalSavings:
        summary?.totalSavings ||
        (summary?.membershipSavings || 0) + (summary?.promotionSavings || 0) ||
        0,
  ...
  }, [cart, summary]); // Only recalculate when cart or summary changes
