...
interface CartState {
  ...
  // Stats (reactive state, not computed)
  stats: CartStats;
...
export const useCartStore = create<CartState>()(
  subscribeWithSelector((set, get) => {
    ...
    const calculateStats = (cart: Cart | null): CartStats => {
      if (!cart?.items) {
        return { totalItems: 0, totalPrice: 0, itemCount: 0 };
      }
    ...
    return {
      ...
      initializing: true,
      error: null,
      stats: { totalItems: 0, totalPrice: 0, itemCount: 0 }, // Initial stats
      ...
      initializeCart: async () => {
        try {
          ...
          if (result.success && result.cart) {
            ...
            set({ cart: result.cart, stats: newStats, initializing: false });
          } else {
            const emptyCart = { items: [] };
            const newStats = calculateStats(emptyCart);
            set({ cart: emptyCart, stats: newStats, initializing: false });
          }
        } catch (error) {
          console.error("Failed to initialize cart:", error);
          const emptyCart = { items: [] };
      ...
      addToCartOptimistic: async (product: any, quantity: number) => {
        ...
        // Apply optimistic update immediately with new stats
        ...
        try {
          ...
          if (result.success && result.cart) {
            // Replace with server response and update stats
      ...
      clearCartOptimistic: async () => {
        ...
        try {
          // Make server call (you'll need to add this action if you haven't)
          // const result = await clearCart();
          // For now, we'll just keep the optimistic update
          toast.success("Cart cleared");
        } catch (error) {
