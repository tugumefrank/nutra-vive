...
//   }
// }
// Updated functions in favouriteServerActions.ts
...
export async function getUserFavorites(): Promise<{
  ...
  try {
    ...
    try {
      ...
      console.log("User not found in database, returning empty favorites");
      return {
        products: [],
        total: 0,
      };
    ...
    // Get favorites with populated product data
    const favorites = await Favorite.find({ user: user._id }) // Use MongoDB ObjectId
      .populate({
        path: "product",
        match: { isActive: true }, // Only include active products
        populate: {
...
export async function clearAllFavorites(): Promise<{
  ...
  try {
    ...
    // Delete all favorites for user
    const result = await Favorite.deleteMany({ user: user._id }); // Use MongoDB ObjectId
...
// Get favorite stats for user (UPDATED VERSION)
export async function getFavoriteStats(): Promise<{
  total: number;
  byCategory: Record<string, number>;
  recent: number; // favorites added in last 7 days
