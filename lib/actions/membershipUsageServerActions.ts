...
export async function deductMembershipUsageOnOrderConfirmation(
  ...
  try {
    ...
    console.log(
      "🔍 Processing membership usage deduction for order:",
      order.orderNumber
    );
    ...
    for (const orderItem of order.items) {
      ...
      const isFreeFromMembership =
        orderItem.price === 0 ||
        (orderItem.appliedDiscount && orderItem.appliedDiscount > 0);
      ...
      if (
        ...
        (orderItem.product as PopulatedProduct).category
      ) {
        const populatedProduct = orderItem.product as PopulatedProduct;
        ...
        if (!product || !product.category) {
          console.warn(
            `⚠️ Product category not found for ${orderItem.productName}`
          );
          continue;
        }
      ...
      if (!userUsage) {
        console.warn(
          `⚠️ Category ${categoryName} not found in user's membership allocations`
        );
        continue;
      }
      ...
      console.log(
        `📦 Will deduct ${orderItem.quantity} of ${orderItem.productName} from ${categoryName} allocation`
      );
    }
    ...
    for (const [categoryId, deduction] of deductionMap) {
      try {
        ...
        if (usageIndex === -1) {
          console.warn(
            `⚠️ Usage tracking not found for category ${deduction.categoryName}`
          );
          continue;
        }
        ...
        const newUsedQuantity =
          currentUsage.usedQuantity + deduction.quantityUsed;
        const newAvailableQuantity = Math.max(
          0,
          currentUsage.allocatedQuantity - newUsedQuantity
        );
        ...
        await UserMembership.findByIdAndUpdate(userMembership._id, {
          $set: {
            ...
            [`productUsage.${usageIndex}.availableQuantity`]:
        ...
        console.log(
          `✅ Deducted ${deduction.quantityUsed} from ${deduction.categoryName}:`,
          {
        ...
        console.error(
          `❌ Failed to update usage for category ${deduction.categoryName}:`,
          updateError
        );
    ...
    console.log(
      `✅ Successfully deducted membership usage for order ${order.orderNumber}:`,
      {
        totalCategories: deductedItems.length,
        deductedItems: deductedItems
          .map((item) => `${item.quantityUsed} ${item.categoryName}`)
          .join(", "),
      }
    );
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to deduct membership usage",
    };
  }
}
...
export async function restoreMembershipUsageOnOrderCancellation(
  ...
  try {
    ...
    console.log(
      "🔄 Restoring membership usage for cancelled order:",
      order.orderNumber
    );
    ...
    for (const [categoryId, restoration] of restorationMap) {
      try {
        ...
        const newUsedQuantity = Math.max(
          0,
          currentUsage.usedQuantity - restoration.quantityUsed
        );
        ...
        console.log(
          `✅ Restored ${restoration.quantityUsed} to ${restoration.categoryName}:`,
          {
            previousUsed: currentUsage.usedQuantity,
            newUsed: newUsedQuantity,
            previousAvailable: currentUsage.availableQuantity,
        ...
        console.error(
          `❌ Failed to restore usage for category ${restoration.categoryName}:`,
          updateError
        );
      }
    }
    ...
    console.log(
      `✅ Successfully restored membership usage for cancelled order ${order.orderNumber}`
    );
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to restore membership usage",
    };
  }
}
