...
//   }
// }
// lib/actions/productserverActions.ts
"use server";
...
// Get Products with Filters - Main function used in product listing
...
// Get Product by Slug - Main function used in product details page
export async function getProductBySlug(slug: string): Promise<IProduct | null> {
  try {
    await connectToDatabase();
  ...
  try {
    ...
    return {
      ...
      product: createdProduct
        ? transformProductDocument(createdProduct)
        : undefined,
    };
  } catch (error) {
    console.error("❌ Error creating product:", error);
  ...
  try {
    ...
    return {
      ...
      product: updatedProduct
        ? transformProductDocument(updatedProduct)
        : undefined,
    };
  } catch (error) {
    console.error("❌ Error updating product:", error);
